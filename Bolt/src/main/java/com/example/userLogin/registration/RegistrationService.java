...
import com.example.userLogin.requests.RegistrationRequest;
...
public class RegistrationService {
    ...
    public String register(RegistrationRequest request) {
        ...
        if (request.getCar()==null){
            token = appUserService.signUpUser(
                    new AppUser(
                            ...
                            Long.valueOf(request.getCardNumber()),
                            request.getCardValidity(),
                            request.getCv(),
        ...
        if(token != null){
            if (!appUserRepository.findByEmail(request.getEmail()).get().getEnabled()){
                String link = "http://localhost:8080/api/v1/registration/confirm?token=" + token;
                emailSender.send(request.getEmail(), buildEmail(request.getFirstName(),link));
                appUserRepository.availability(false, request.getEmail());
            }
        }
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                ConfirmationToken confirmationToken = confirmationTokenService.
                    getToken(token)
                    .orElseThrow(() -> new IllegalStateException("t not found"));
                ...
                if (confirmationToken.getConfirmedAt() == null){
                    ...
                    System.out.println("email already confirmedT");
                }
            }
        };
    ...
    public String confirmToken(String token) {
        ConfirmationToken confirmationToken = confirmationTokenService
                .getToken(token)
                .orElseThrow(() ->
                        new IllegalStateException("Token not found or already expired. Please register your account again."));
